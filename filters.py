...
def filter_by_time(time1, time2):
    global var_in_class
    var_in_class = var_in_class[var_in_class.time.dt.time.between(time1, time2)]
...
def filter_by_area(t_l, b_r):
    global var_in_class
    var_in_class = var_in_class[(var_in_class.x.between(t_l[0], b_r[0])) & (var_in_class.y.between(t_l[1], b_r[1]))]
    # return table[['x', 'y']]
